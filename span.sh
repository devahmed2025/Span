...
if [[ "$-" == *x* || "$-" == *v* ]] || [[ $(ps -o comm= $(cat /proc/$$/status | awk '/TracerPid/ {print $2}')) ]]; then
    exec >/dev/null 2>&1  # Hide all output if debugging is detected
fi
EE="2025-07-28"
...
echo -e "\nThis script was Created by Ahmed-Elshahhat to Calculate span-loss for Etisalat Network 2025 Version\n"
sleep 2
...
output="/Span_Loss_New"
csv_file="$output/Amplifier_Power_Values_$timestamp.csv"
